...
export function help() {
    ...
    new TextComponent(mesgui).setClick("suggest_command", `${gui}`).setHover("show_text", `&bクリックしてコマンドを貼り付け。`).chat()
    new TextComponent(mesadd).setClick("run_command", `/helpcommandImsun`).setHover("show_text", `&bクリックして見本を表示。`).chat()
    new TextComponent(meslis).setClick("run_command", `/sun list`).setHover("show_text", `&bクリックしてコマンドを実行。`).chat()
